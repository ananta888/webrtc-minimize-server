...
const base = query => ({ ...query, type: "source-program-scene", expectedSceneRevision: 1,
  layout: "waiting-slate", sourceLeaseIds: [], activeSourceLeaseId: "", ...(query.version === 2 ? { sourceFits: [] } : {}) });
...
function correlation(value, request, type, fields, now) {
  if (!closed(value, ["version", "type", ...scope, "observedAt", ...fields]) || value.version !== request.version
...
export function normalizeNativeSourceSceneState(value, query, now = Date.now()) {
  ...
  correlation(value, request, "source-program-scene-state",
    ["sceneRevision", "layout", "sourceLeaseIds", "activeSourceLeaseId", "availableSources", ...(request.version === 2 ? ["sourceFits"] : [])], now);
  ...
  normalizeNativeSourceScene({ ...base(request), layout: value.layout, sourceLeaseIds: value.sourceLeaseIds,
    activeSourceLeaseId: value.activeSourceLeaseId, ...(request.version === 2 ? { sourceFits: value.sourceFits } : {}) }, now);
  ...
  return Object.freeze({ ...value, sourceLeaseIds: Object.freeze([...value.sourceLeaseIds]),
    ...(request.version === 2 ? { sourceFits: Object.freeze([...value.sourceFits]) } : {}),
