...
for utility in curl sha256sum timeout flock systemctl; do command -v "$utility" >/dev/null || die 'Required maintenance utility is missing.'; done
...
flock -n 9 || die 'Another maintenance operation is active.'
...
restore_work() {
  ...
  mv -Tf -- "$root/$restore_ref/restore" "$binary" || return 1
...
systemctl --user is-active --quiet "$service" || die 'The installed user service must be active before updating.'
old_ref=''
...
pending=1
printf '%s\n' "$work_ref" > "$work/ref"
mv -Tf -- "$work/ref" "$root/.update-active"
stop_service
mv -Tf -- "$work/candidate" "$binary"
start_service || die 'Candidate service did not remain active.'
printf '%s\n' "$work_ref" > "$work/ref"
mv -Tf -- "$work/ref" "$root/.rollback-ref"
committed=1
