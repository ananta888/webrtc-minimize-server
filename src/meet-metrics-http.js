...
export class MeetMetricsHttp {
  ...
  async read(request, url) {
    ...
    if (this.#closed || config.meetMetricsEnabled !== true || config.authMode !== "required"
      || !config.publicOrigin?.startsWith("https://") || request.method !== "GET"
      || url.pathname !== "/api/meet/metrics" || url.search
      || (request.headers.origin !== undefined && request.headers.origin !== config.publicOrigin)) {
      return reply(404);
    }
    const release = this.#acquire();
    if (!release) return reply(429, "meet_metrics_temporarily_unavailable\n");
    try {
      ...
      return body ? reply(200, body) : reply(503, "meet_metrics_unavailable\n");
    } catch (error) {
      if (error instanceof AuthenticationError) return reply(error.code === "broadcast_operator_required" ? 403 : 401);
      return reply(503, "meet_metrics_unavailable\n");
    } finally { release(); }
  }
