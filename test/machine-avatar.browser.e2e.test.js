...
test(`${humanEngine} decodes independent neutral avatar alongside speech and screen without capture`, { timeout: 90000 }, async t => {
  ...
  const first = await machine.evaluate(id => window.anantaMachine.avatar.open(id, "neutral-ai-v1"), sourceId);
  ...
  await machine.evaluate(() => window.__avatarCompanions.close());
  const stale = await machine.evaluate(id => window.anantaMachine.avatar.open(id, "neutral-ai-v1"), sourceId);
  ...
  const fresh = await machine.evaluate(id => window.anantaMachine.avatar.open(id, "neutral-ai-v1"), sourceId);
  ...
  t.diagnostic(JSON.stringify({ synthetic: true, productionEvidence: false, image, audioWindows: audio.active_windows, movingIndicator: changed }));
