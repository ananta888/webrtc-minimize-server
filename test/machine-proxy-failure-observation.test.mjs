...
test("proxy failure snapshot executes only two bounded read-only operations for its exact fixture", () => {
  ...
  assert.deepEqual(value, { container: { status: "running", running: true, oomKilled: false, exitCode: 0 }, listenerAnnounced: true });
...
test("failed, malformed, oversized and non-string reads remain unknown, never healthy", () => {
  ...
  assert.deepEqual(observeMachineProxyFailure(name, () => { throw Error("private docker command"); }),
    { container: null, listenerAnnounced: null });
});
...
test("a listener announcement is historical, not proof of current process health", () => {
  ...
  assert.deepEqual(value, { container: { status: "exited", running: false, oomKilled: true, exitCode: 137 }, listenerAnnounced: true });
...
test("an actual stuck inspection is killed without retaining its partial output or changing the original failure", { timeout: 4000 }, () => {
  ...
  assert.equal(calls, 2); assert.deepEqual(value, { container: null, listenerAnnounced: false });
