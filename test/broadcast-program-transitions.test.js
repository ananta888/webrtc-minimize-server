...
import { transitionMetricSamples, hostResourceMetricSamples, hostResourceCounts } from "../src/broadcast-metric-samples.js";

const NOW = 1_800_000_000_000, tenant = "tn_aaaaaaaaaaaaaaaa", program = "prg_aaaaaaaaaaaaaaaa";
const record = (state, { epoch = 1, pending = false, id = program } = {}) => ({ snapshot: { machine: { scope: { tenantId: tenant, programId: id },
  program: { state, revision: 1, programEpoch: epoch } } }, pendingHandoff: pending ? {} : null });
const seconds = (t, transition, at) => t.samples(at).filter(s => s.transition === transition).map(s => s.seconds);

test("start, stop and handoff durations are derived from committed transitions only, without identifiers", () => {
  ...
  assert.equal(t.observe(record("live", { epoch: 2 }), record("stopping", { epoch: 2 }), NOW + 6000), true);
  assert.equal(t.observe(record("stopping", { epoch: 2 }), record("stopped", { epoch: 2 }), NOW + 6250), true);
  assert.deepEqual(seconds(t, "stop", NOW + 6250), [.25]);
  assert.equal(t.pendingPrograms, 0, "a stopped program keeps no anchor");
  assert.equal(t.observe(record("stopped", { epoch: 2 }), record("stopped", { epoch: 2 }), NOW + 7000), true);
  ...
  assert.doesNotMatch(JSON.stringify(t.samples(NOW + 8000)), /prg_|tn_|epoch|revision/);
});
...
test("transition and host samples are validated atomically and cannot invent zeros", () => {
  for (const bad of [[{ transition: "start", seconds: -1 }], [{ transition: "reboot", seconds: 1 }], [{ transition: "start", seconds: 3601 }],
    [{ transition: "start", seconds: 1, programId: program }], Array(257).fill({ transition: "stop", seconds: 1 }), "start"]) {
