...
/** Actual production binaries, no extra native control/decrypt test endpoint. */
export async function nativeSceneProcesses(t) {
  ...
  for (const [name, module] of [["packager", "native-broadcast-packager"], ["origin", "broadcast-hls-origin"]]) {
    ...
    try {
      await execute(localGo ? "go" : "docker", localGo ? ["build", "-o", path.join(directory, name), "."] : [
        "run", "--rm", "--name", container, "-e", "CGO_ENABLED=0",
        "-v", `${path.join(root, module)}:/src:ro`, "-v", `${directory}:/out`, "-w", "/src",
        "golang:1.24-alpine", "go", "build", "-o", `/out/${name}`, ".",
      ], {
...
export function startSceneProcess(executable, environment, create = spawn) {
  const child = create(executable, [], { env: environment, stdio: "ignore", detached: false });
