...
import { machinePublicDirectory } from "./machine-public-directory.mjs";

export async function machineBrowserFixture(t, { listenHost = "127.0.0.1", listenPort = 0, hubPublicKey, tlsPortProxy = false,
  lifetimeSeconds = 180, humanEngine = "chromium", observeStage = () => {}, publicDir = process.env.MEET_TEST_PUBLIC_DIR } = {}) {
  ...
  publicDir = await machinePublicDirectory(publicDir); // Fail before creating resources.
  ...
  observeStage("signaling-server"); app = createAppServer({ config, oidcVerifier, publicDir });
