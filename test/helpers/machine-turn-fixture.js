...
export function privateMachineTurn({ network, address, lifetimeSeconds, transport }, run) {
  if (!/^meet-test-tls-[a-f0-9-]{36}-network$/.test(network) || isIP(address) !== 4
    ...
    || !["udp", "tcp"].includes(transport)) throw new Error("test_turn_scope_invalid");
  ...
  return {
    ...
    start() {
      ...
      run(["create", "--name", name, "--network", network, "--ip", address,
        ...
        "--total-quota=16", "--user-quota=8", "--bps-capacity=4000000", "--max-bps=1000000",
