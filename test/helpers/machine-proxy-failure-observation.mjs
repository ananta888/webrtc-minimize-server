...
import { projectMachineProxyWait } from "./machine-proxy-wait.mjs";
...
export function observeMachineProxyFailure(name, execute = execFileSync) {
  ...
  const resources = projectMachineProxyResources(read(["exec", name, "cat", "/proc/1/stat",
    "/sys/fs/cgroup/cpu.stat", "/sys/fs/cgroup/memory.events", "/proc/1/io"]));
  const waitSymbol = projectMachineProxyWait(read(["exec", name, "cat", "/proc/1/wchan"]));
  let container = null;
  try {
  ...
  return Object.freeze({ container, processEntered: announced("test_tls_process_entered"),
    networkModuleLoaded: announced("test_tls_network_module_loaded"), listenerAnnounced: announced("test_tls_listener_ready"), resources, waitSymbol });
}
