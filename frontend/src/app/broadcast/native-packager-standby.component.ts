...
export class NativePackagerStandbyComponent implements OnChanges, OnDestroy {
  ...
  readonly canSave = computed(() => !this.disabled() && !this.standby.busy() && this.changed()
  ...
  setRenditions(value: string): void { if (["1", "2", "3"].includes(value)) this.renditions.set(Number(value)); }
  ...
  async save(): Promise<void> {
    ...
    const control = this.standby.control(), selected = JSON.stringify(this.standby.selected()), renditions = this.renditions();
    ...
    if (!this.canSave() || this.programId() !== programId || this.programEpoch() !== epoch
      ...
      || this.renditions() !== renditions) return;
    try { await this.standby.save(renditions, "user-action"); } catch { /* Server state must be loaded again after scope loss. */ }
  }
}
