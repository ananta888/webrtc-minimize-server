...
interface SceneScope {
  readonly sceneControlVersion: 1; readonly programId: string; readonly programRevision: number;
...
export function parseNativeSceneResult(raw: unknown, program: { programId: string; programRevision: number; programEpoch: number }, now = Date.now()): NativeSceneResult {
  ...
  if (!closed(v, fields) || v.sceneControlVersion !== 1 || !ref(v.programId, "prg") || v.programId !== program.programId
