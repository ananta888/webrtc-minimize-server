...
it("renders real controls and separate audio/video demand only after a click, then removes stale values", async () => {
  const session = signal<string | null>("session-one");
  const programs = { capacityContext: () => session(), previewCapacity: vi.fn(async () => ({
    schema: "ananta.native-capacity-preview.v1", reserved: false, costStatus: "unknown", observedAt: 1800000000000,
