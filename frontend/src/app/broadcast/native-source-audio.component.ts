...
import { NativeAudioLevel, NativeAudioSelection } from "./native-source-audio-contract";
...
export class NativeSourceAudioComponent {
  ...
  readonly status = computed(() => ({ idle: "Audiowerte noch nicht abgefragt.", pending: "Warte auf den aktuellen Packager…",
    ...
    unavailable: "Keine verlässliche Bestätigung. Rechte, Sendung und Packager mit Audio-Steuerung v1 prüfen. Kein automatischer Wiederholungsversuch.",
  ...
  private selection(): NativeAudioSelection | null {
    ...
    return audio ? { expectedAudioRevision: audio.audioRevision, sources: this.levels().map(s => ({ ...s })) } : null;
  }
  async apply(): Promise<void> {
    const selected = this.selection();
    if (this.audio.view().phase !== "ready" || !selected?.sources.length) return;
    if (!window.confirm("Pegel und Stummschaltung im Broadcast-Ausgang ändern? Raumton und Quellenfreigaben bleiben unverändert. Bereits ausgelieferter Ton kann nicht zurückgerufen werden.")) return;
