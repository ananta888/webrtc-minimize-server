...
export class MachineChatSessionService implements OnDestroy {
  ...
  private authority(): MachineChatAuthority {
    ...
    if (!context || !lease || !this.session.joined() || !this.mesh.machineReceive.isMachine(own)
      || !this.mesh.machineReceive.supports(own, "chat.read")) throw new Error("meet_chat_receive_denied");
    const scope = validateMachineChatScope({ origin: location.origin, tenant_id: context.tenantId,
      ...
      policy_revision: Math.max(1, this.mesh.machineReceive.revision()), deadline_ms: lease.expiresAt });
    return Object.freeze({ chatRead: true, chatSend: this.mesh.machineReceive.supports(own, "chat.send"), scope });
  }
