...
const response = (value: unknown = contract()) => new Response(JSON.stringify(value), { headers: { "content-type": "application/json" } });
...
describe("machine admission status (observation, never authority)", () => {
  ...
  it.each([false, true])("reads admission %s without credentials, follows no redirects and expires without polling", async enabled => {
    vi.useFakeTimers(); const fetcher = vi.fn(async () => response(contract(enabled))); vi.stubGlobal("fetch", fetcher);
    ...
    expect(fetcher).toHaveBeenCalledExactlyOnceWith("/api/machine/capabilities", expect.objectContaining({
  ...
  it.each(["http", "redirect", "type", "length", "stream", "json", "schema", "utf8", "missing-body"])("clears earlier success on invalid %s", async kind => {
    vi.useFakeTimers(); const fetcher = vi.fn(async () => response(contract(true))); vi.stubGlobal("fetch", fetcher);
    ...
    if (kind === "schema") bad = response({ ...contract(), future: true });
  ...
  it("allows one request, enforces its deadline even without cooperative fetch, and ignores a late response", async () => {
    ...
    resolve(response(contract(true))); await Promise.resolve(); await Promise.resolve();
