...
Ab `MEDIA_AGENT_MIN_PARTICIPANTS` (Default 3) darf genau ein gesunder, consentierter Primary den Publisher-Fanout reduzieren; ein Zweiergespräch bleibt direkt. Unterhalb der Shard-Schwelle – mit den Defaults also in Räumen mit drei bis fünf Teilnehmern – werden freie Kapazität, aktuelle Last, Netz- und Batterieklasse sowie Creator-Präferenz bewertet, aber noch keine Publisher auf mehrere Agenten verteilt. Ab `MEDIA_AGENT_SHARD_MIN_PARTICIPANTS` weist die Control Plane jedem Publisher einen Ingress und jedem Subscriber einen Egress zu und kann Primary plus höchstens zwei Standbys als Forwarder nutzen. Bei mehreren aktiven Agenten erzeugt sie einen epochgebundenen, zyklenfreien DAG mit höchstens zwei Hops. Die Agenten bauen die autorisierten Links direkt per WebRTC auf und transportieren ausschließlich die angeforderten SFrame-Ciphertext-Layer; SDP/ICE läuft über die Control Plane, Medien nicht. Bis E2EE-Key-ACK, Agent-/Browser-Verbindung, angewendeter Layer und Receiver-ACK vollständig bestätigt sind, bleibt das required-SFrame-Direkt-Mesh aktiv. Widerruf, fehlende Kapazität oder eine abgelaufene Lease führen ohne Klartextfallback zum Direct-SFrame-Pfad zurück. Kein Agent erhält SFrame-Schlüssel, Decrypt-Port, Membership- oder Policy-Autorität. Die reale Produktionsmatrix aus BME-006 bleibt trotzdem ausdrücklich kein garantierter QoS-Nachweis.
