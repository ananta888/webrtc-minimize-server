	...
	for _, ending := range []string{"stop", "disconnect", "cancel", "duplicate-auth", "disabled", "unauthenticated"} {
		t.Run(ending, func(t *testing.T) {
			...
			if c.cfg.sourcePrograms {
				if capability["capabilityVersion"] != float64(6) || capability["sourcePrograms"] != true || capability["sourceAudioControlVersion"] != float64(3) || capability["sourceAudioEncodingVersion"] != float64(1) || capability["sourceSceneControlVersion"] != float64(2) {
