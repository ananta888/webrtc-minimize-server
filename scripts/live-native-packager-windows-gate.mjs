...
// This gate executes only synthetic config/output tests, never enrollment,
...
try {
  ...
  const result = execFileSync("powershell.exe", ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command",
    `& ${psQuote(windowsPath)} '-test.run=^(TestConfig|TestTranscodeOutput|TestOutput)' '-test.v'; exit $LASTEXITCODE`],
  ...
  console.log("PASS real Windows config/output tests; FFmpeg media, installation, autostart and signing remain separate gates.");
} catch (error) {
